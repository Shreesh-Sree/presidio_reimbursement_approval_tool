...
def list_categories(
    ...
    user: dict[str, str] = Depends(require_permission("category:read")),
):
    categories = category_service.list_categories(
        db, user["organization_id"], include_archived=include_archived
    )
...
def create_category(
    ...
    db: Session = Depends(get_db),
    user: dict[str, str] = Depends(require_permission("category:manage")),
):
    try:
...
def get_category(
    ...
    try:
        return category_service.category_payload(
            category_service.get_category(db, category_id, user["organization_id"])
        )
    except Exception as exc:
        _category_error(exc)
...
def update_category(
    ...
    try:
        ...
        category = category_service.update_category(
            db, category_id, organization_id=user["organization_id"], **changes
        )
        return category_service.category_payload(category)
    except Exception as exc:
        _category_error(exc)
...
def delete_category(
    ...
    try:
        category_service.deactivate_category(db, category_id, user["organization_id"])
...
def restore_category(category_id: str, db: Session = Depends(get_db), user: dict[str, str] = Depends(require_permission("category:manage"))):
    try:
        return category_service.category_payload(
            category_service.restore_category(db, category_id, user["organization_id"])
        )
    except Exception as exc:
        _category_error(exc)
...
def permanently_delete_category(category_id: str, db: Session = Depends(get_db), user: dict[str, str] = Depends(require_permission("category:manage"))) -> Response:
    try:
        category_service.permanently_delete_category(db, category_id, user["organization_id"])
        return Response(status_code=status.HTTP_204_NO_CONTENT)
    except Exception as exc:
        _category_error(exc)
