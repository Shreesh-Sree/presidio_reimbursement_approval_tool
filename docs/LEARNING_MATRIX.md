...
| SQL joins, indexing, transactions | SQLAlchemy services and Alembic migrations implement normalized relationships; `004_payment_operations`, `005_delegated_approvals`, and `006_scope_policies_to_organizations` add queue, batch, due-date, audit, and tenant indexes. | Add `EXPLAIN` notes and measured composite indexes when report lists or approval queues show a real query need.  Use one transaction for each payment/approval state transition and audit event. |
...
| Authentication and authorization | JWT validation is in `app/core/security.py`; `require_permission()` enforces RBAC. | Current choice is token-based auth.  Evaluate OAuth2/OIDC/SSO only with an identity-provider decision record; do not mix browser sessions and bearer tokens without an explicit threat model. |
...
| Browser storage, JWT, and CSRF | The current client stores a bearer token in local storage. | Document the XSS trade-off and plan an HttpOnly-cookie/CSRF migration before handling higher-risk enterprise tenants.  Do not claim local storage is equivalent to a secure server session. |
...
| Least privilege and secrets | Terraform limits runtime IAM actions and separates core/AI Secrets Manager values; the AI service rejects reuse of the core database. | Use OIDC for any future GitHub-to-AWS deployment role.  Never commit `.env`, database URLs, provider keys, Terraform state, or generated credentials. |
...
| Structured outputs | AI-review and policy-assistant contracts use strict Pydantic models; the policy assistant returns source-grounded citations and an explicit insufficient-evidence response.  Gemini narrative text remains free form. | Add a validated Gemini response schema containing only finding/rule references supplied in its prompt; invalid output must fall back to the deterministic narrative. |
